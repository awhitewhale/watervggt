...
def tensor_to_cpu_numpy(obj):
    """递归将tensor转换为CPU numpy数组"""
    if torch.is_tensor(obj):
        return obj.cpu().numpy()
    elif isinstance(obj, dict):
        return {key: tensor_to_cpu_numpy(value) for key, value in obj.items()}
    elif isinstance(obj, list):
        return [tensor_to_cpu_numpy(item) for item in obj]
    elif isinstance(obj, tuple):
        return tuple(tensor_to_cpu_numpy(item) for item in obj)
    else:
        return obj
...
def gradio_demo(
    ...
    cpu_predictions = tensor_to_cpu_numpy(predictions)
    
    np.savez(prediction_save_path, **cpu_predictions)
